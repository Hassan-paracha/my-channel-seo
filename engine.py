...
# --- CONFIGURATION (PHASE 1 RESTORE) ---
CHANNEL_HANDLE = "@HassanParacha-c3g"  # Your active @handle[span_3](start_span)[span_3](end_span)
DB_FILE = "video_db.json"
DAILY_INDEX_LIMIT = 18                 # Throttled down to ensure organic growth recovery[span_4](start_span)[span_4](end_span)
BATCH_SIZE = 3                         # Safe, human-like batching[span_5](start_span)[span_5](end_span)
COOLDOWN = 120                         # 2 minutes delay between batches to protect domain authority[span_6](start_span)[span_6](end_span)
...
def run_rolling_cycle():
    ...
    # Grab the oldest refreshed entries to stay within the safe daily limit
    ...
    # Batch processing with human-paced throttling loops
    for i in range(0, len(to_process), BATCH_SIZE):
        batch = to_process[i:i+BATCH_SIZE]
        db_changed = False
        
        for v_id in batch:
            old_title = db[v_id]['title']
            new_title = ai_polish(old_title)
            
            # Update values and only overwrite timestamps cleanly
            if new_title != old_title:
                db[v_id]['title'] = new_title
                db[v_id]['last_refreshed'] = today_str
                db_changed = True
            else:
                # Cycle timestamp forward anyway so it moves down the processing queue
                db[v_id]['last_refreshed'] = today_str
                db_changed = True
    ...
    # Sort items cleanly for the static deployment layout
    sorted_for_display = sorted(db.items(), key=lambda x: x[1]['last_refreshed'], reverse=True)
    
    cards_html = ""
    for vid, v in sorted_for_display:
        # Prevent quotes inside titles from snapping the schema string structure
        ...
        <div class="card">
            ...
            {{
              ...
              "uploadDate": "{v['last_refreshed']}T00:00:00Z",
